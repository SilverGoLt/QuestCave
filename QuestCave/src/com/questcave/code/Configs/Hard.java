...
public class Hard {
    ...
    public Hard(Main main) {
  	  file = new File(main.getDataFolder(), "HardQuests.yml");
  	 if (!file.exists()) {
  		 try {
				 file.createNewFile();
	...
	public static ArrayList<Quest> getQuests() {
    	...
    	for (String path:paths) {
    		Quest quest = new Quest(config.getString("quests" + path + ".name"),(ArrayList<Goal>) config.get("quests" + path +".goals"),
    				Difficulty.HARD,(ArrayList<ItemStack>) config.get("quests" + path +".ItemPrize"), config.getInt("quests" + path +".XPPRIZE"));
